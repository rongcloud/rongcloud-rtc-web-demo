...
let room = null;
let roomPKHandler = null;
let pkRoomId = ref('');
let pkRoom = null;
let pkUserId = ref('');
let pkUserIds = reactive([]);
let userIds = reactive([]);
let remoteTracks = reactive([]);
let linkedUserIds = reactive([]);
let volume = reactive({});
...
const joinRoom = async () => {
  const res = await rtc.joinLivingRoom(form.roomId);
  room = res.room;
  const resPK = room.getRoomPKHandler();
  roomPKHandler = resPK.roomPKHandler;
  userIds.length = 0;
  ...
  initPKRoomListener(roomPKHandler);
...
const leaveRoom = async () => {
  await unpublishAll();
  await rtc.leaveLivingRoom(room);
  onRoom.value = false;
  userIds.length = 0;
  remoteTracks.length = 0;
};
...
const initPKRoomListener = (room) => {
  console.log(room)
  room.registerRoomPKEventListener({
    /**
     * 收到连麦邀请，此时可调 roomPKHandler.responseJoinOtherRoom 响应连麦
     */
    onRequestJoinOtherRoom (info) {
      ElMessageBox.confirm(`房间:${info.inviterRoomId}中的用户:${info.inviterUserId}发来跨房间PK邀请，是否同意`).then(async () => {
        room.responseJoinOtherRoom(info.inviterRoomId, info.inviterUserId, true);
        joinOtherRoom(info.inviterRoomId);
      }).catch(() => {
        room.responseJoinOtherRoom(info.inviterRoomId, info.inviterUserId, false);
      });
    },
    /**
     * 收到连麦邀请已被取消
     ...
     */
    onResponseJoinOtherRoom: async (info) => {
      if (info.agree) {
        joinOtherRoom(info.inviteeRoomId);
      }
    },
    /**
     * 收到连麦结束通知
...
const startPk = async () => {
  console.log('startPK', pkRoomId.value, pkUserId.value);
  const { code } = await roomPKHandler.requestJoinOtherRoom(pkRoomId.value, pkUserId.value);
  if (code === RCRTCCode.SUCCESS) {
    console.log('发送连麦邀请成功');
  }
};
const stopPk = async () => {
  const {code} = await roomPKHandler.leaveOtherRoom(pkRoom, true);

  if (code === RCRTCCode.SUCCESS) {
    console.log('结束PK');
    let newUserIds = new Set([...userIds]);

    for (let user of pkUserIds) {
      if (newUserIds.has(user)) {
        newUserIds.delete(user);
      }
    }
...
const joinOtherRoom = async (roomId) => {
  const res = await roomPKHandler.joinOtherRoom(roomId);
  const { room: otherRoom, userIds: joinUserIds, tracks } = res;
  pkRoom = otherRoom;
  pkUserIds = joinUserIds;
  let newUserIds = new Set([...userIds, ...joinUserIds]);

  userIds.length = 0;
  userIds.push(...newUserIds);
  initRTCRoomListener(otherRoom);
  subscribe(otherRoom, tracks);
};
</script>
        ...
        <div>
          ...
          >
            <el-form-item label="房间号">
              <el-input v-model="pkRoomId"></el-input>
            </el-form-item>
            <el-form-item label="用户ID">
              <el-input v-model="pkUserId"></el-input>
            </el-form-item>
            <el-form-item>
              <el-button type="primary" @click="startPk">跨房间连麦</el-button>
              <el-button type="primary" @click="stopPk">结束跨房间连麦</el-button>
            </el-form-item>
          </el-form>
        </div>
      </div>
      ...
      <ul class="track-list">
        ...
        <template v-for="userId in userIds">
          <li>
            ...
            <!-- <div class="status">
              <el-button type="primary" size="small" text>发起连接</el-button>
              <el-button type="primary" size="small" text>取消连接</el-button>
            </div> -->
          </li>
        </template>
      </ul>
